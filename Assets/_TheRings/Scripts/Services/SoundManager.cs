...
namespace SgLib
{
    ...
    public class SoundManager : MonoBehaviour
    {
        ...
        public delegate void OnVibrateStatusChanged(bool isVibrate);

        public static event OnMuteStatusChanged MuteStatusChanged;
        public static event OnVibrateStatusChanged VibrateStatusChanged;
        ...
        private const string VIBRATE_PREF_KEY = "VibratePreference";

        private const int MUTED = 1;
        private const int UN_MUTED = 0;
        private const int VibrateOff = 1;
        private const int VibrateOn = 0;
        private const string MUSIC_PREF_KEY = "MusicPreference";
        private const string VIBRATEONOFF_PREF_KEY = "VibOnOffPreference";
        private const int MUSIC_OFF = 0;
        private const int MUSIC_ON = 1;

        private const int VIBRATE_OFF = 0;
        private const int VIBRATE_ON = 1;
        ...
        {
            return (PlayerPrefs.GetInt(VIBRATE_PREF_KEY, VibrateOn) == VibrateOff);
        }

        public bool IsVibrateOff()
        {
            return (PlayerPrefs.GetInt(VIBRATEONOFF_PREF_KEY, VIBRATE_ON) == VIBRATE_OFF);
        }



        ///<summary>
        /// Toggles vibrate on off status
        /// </summary>
        public void ToggleVibrate()
        {
            // Toggle current mute status
            isVibrate = !IsVibrateOn();

            if (isVibrate)
            {
                // Muted
                PlayerPrefs.SetInt(VIBRATE_PREF_KEY, VibrateOff);

                Debug.Log(VibrateOff);


                if (VibrateStatusChanged != null)
                {
                    VibrateStatusChanged(true);
                }
            }
            else
            {
                // Un-muted
                PlayerPrefs.SetInt(VIBRATE_PREF_KEY, VibrateOn);

                Debug.Log(VibrateOn);
                if (VibrateStatusChanged != null)
                {
                    VibrateStatusChanged(false);
                }
            }
            Debug.Log(isVibrate);
